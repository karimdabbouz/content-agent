...
from pydantic_ai.mcp import MCPServerSSE, MCPServerHTTP, MCPServerStreamableHTTP
import asyncio
...
async def main():
    async with agent:
        result = await agent.run('get the content of the wikipedia article on dortmund.')
    print(result.output)
